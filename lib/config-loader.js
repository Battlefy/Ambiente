...
function ConfigLoader(configPath, defaultsPath, callback) {
  ...
  var next = function(err) {
    ...
    _this._gatherConfigs(_this.path, function(err) {
  ...
  if(defaultsPath) { this._gatherConfigs(this.defaultsPath, next); }
  else { next(); }
}
      ...
      dir.forEach(function(fileName) {
        ...
        fs.stat(filePath, function(err, stat) {
          ...
          // else if the file is a json file
            ...
            var config = _this._loadConfig(filePath);
            _this._configs[namespace + '.' + fileId] = config;
            rootObj[fileId] = config;
            next();
          }
...
ConfigLoader.prototype._loadConfig = function(configPath) {
